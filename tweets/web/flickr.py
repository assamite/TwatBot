...
2014 
  Small patches to here and there by Simo Linkola
...
class Photo(object):
    ...
    __readonly = ['isfavorite', 'license', 'rotation', 
                  'dateposted', 'datetaken', 'takengranularity', 
                  'ispublic', 'isfriend', 'isfamily', 
                  'cancomment', 'canaddmeta', 'comments', 'permcomment', 
    ...
    def __init__(self, id, owner=None, dateuploaded=None, \
                 ...
                 rotation=None, url=None, views=None, machine_tags = None,\
                 owner_name = None):
        ...
        self.url_s = self.getSmallSquare()
        self.url_z = self.getMedium()
        self.url_q = self.getSemiSmallSquare()
    ...
    def getSemiSmallSquare(self):
        """
        Return a string representation of the URL to the small square
        image (not the small square image page).
        """
        return self._getDirectURL('q')
...
def photos_search(user_id='', auth=False,  tags='', tag_mode='', text='',\
                  ...
                  license='2', per_page=500, page='', sort='relevance',\
                  safe_search='', content_type='', media='photos',\
                  extras='machine_tags,tags,description,owner_name', **kwargs):
...
def photos_search_pages(user_id='', auth=False,  tags='', tag_mode='', text='',\
                  min_upload_date='', max_upload_date='',\
                  min_taken_date='', max_taken_date='', media = 'photos',\
                  license='2', per_page='500', page='', sort='relevance',\
                  extras = 'machine_tags,tags,description,owner_name'):
...
def _parse_photo(photo):
    ...
    p = Photo(photo.id, owner=owner, title=title, ispublic=ispublic,\
              ...
              server=server, farm=farm, tags = tags, owner_name = owner_name,\
              machine_tags = machine_tags, description = description)        
    return p
